...
const LISTS: Checklist[] = [
  ...
  {
    id: 'failsafe',
    title: 'Failsafe — за роллю борту',
    hint: 'Поведінка при втраті лінку: ударний ≠ розвідувальний',
    items: [
      'Throttle Channel Fallback = Hold (НЕ Auto — інакше мотори глохнуть у Stage 1)',
      'Stage 1 guard time виставлено (≈0.5–1.0 с) — коротке утримання перед Stage 2',
      'УДАРНИЙ борт: Stage 2 = Drop (миттєвий disarm) — не повертається й не «гуляє» над своїми',
      'РОЗВІДУВАЛЬНИЙ борт: Stage 2 = GPS Rescue / RTH, GPS fix стабільний',
      'GPS Rescue (крило/розвідка): висота, швидкість і home-точка виставлені й перевірені',
      'Failsafe на приймачі (ELRS «no pulses» / set failsafe) узгоджено з Betaflight',
      'Failsafe ПРОТЕСТОВАНО: вимкнути пульт — борт реагує як налаштовано (БЕЗ пропелерів!)',
    ],
  },
  {
    id: 'opsec',
    title: 'OPSEC / гігієна даних',
    hint: 'Щоб борт не здав позицію, маршрут чи підрозділ',
    items: [
      'Blackbox очищено перед вильотом (лог містить GPS-трек — може видати маршрут і позицію)',
      'GPS-поля в blackbox вимкнено, якщо лог не потрібен (економія + менше даних на борту)',
      'Координати/домашню точку не лишати у відкритому OSD та збережених логах',
      'Craft name / ELRS binding phrase без прив’язки до підрозділу чи позивних',
      'SD-карта / flash очищені перед передачею борту далі по ланцюгу',
      'Після втрати борту — конфіг і частоти вважати скомпрометованими, змінити на парку',
      'Фото/відео збірок — без геоприв’язки, без впізнаваного тла локації',
    ],
  },
];
