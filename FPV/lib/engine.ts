...
export interface Component {
  id: string;
  type: 'receiver' | 'vtx_digital' | 'vtx_analog' | 'gps';
  brand: string;
  model: string;
  protocol?: string;
  requirements: {
    connection: 'uart' | 'uart_tx_only';
...
export function buildPlan(board: Board, components: Component[]): Plan {
  ...
  const ok = assignments.every((a) => a.uart !== null);
